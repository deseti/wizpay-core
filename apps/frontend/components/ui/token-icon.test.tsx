...
import { ARC_MAINNET_CHAIN_ID, SUPPORTED_TOKENS } from "@/lib/wizpay";
import { TokenIcon } from "./token-icon";

describe("TokenIcon", () => {
  it("renders a local canonical image and keeps fixed geometry", () => {
    const { container } = render(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={SUPPORTED_TOKENS.USDC.address} symbol="FAKE" size={32} decorative={false} />);
  ...
  it("renders canonical EURC from its address, not the supplied symbol", () => {
    const { container } = render(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={SUPPORTED_TOKENS.EURC.address} symbol="USDC" />);
  ...
  it("resets a prior image error when the resolved source changes", () => {
    const { container, rerender } = render(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={SUPPORTED_TOKENS.USDC.address} symbol="USDC" />);
    fireEvent.error(container.querySelector("img")!);
    expect(screen.getByText("USDC")).toBeInTheDocument();
    rerender(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={SUPPORTED_TOKENS.EURC.address} symbol="EURC" />);
    expect(new URL(container.querySelector("img")?.getAttribute("src") ?? "", window.location.href).pathname).toBe("/tokens/eurc.png");
    expect(screen.queryByText("EURC")).not.toBeInTheDocument();
  });

  it("falls back to address-derived initials after an image error", () => {
    const { container } = render(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={SUPPORTED_TOKENS.EURC.address} symbol="USDC" size={24} />);
    fireEvent.error(container.querySelector("img")!);
  ...
  it("uses a deterministic fallback for an unknown address", () => {
    render(<TokenIcon chainId={ARC_MAINNET_CHAIN_ID} address={`0x${"1".repeat(40)}`} symbol="ABC" />);
