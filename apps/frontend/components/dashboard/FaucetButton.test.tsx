...
vi.mock("@/components/providers/CircleWalletProvider", () => ({
  useCircleWallet: () => ({
    ...
    sepoliaWallet: state.sepoliaAddress
      ? { address: state.sepoliaAddress }
      : null,
  }),
}));
...
describe("FaucetButton wallet presentation", () => {
  beforeEach(() => {
  ...
  it("renders exactly one canonical EVM Address and no network or Solana cards", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Copy EVM Address" }),
    ).toHaveAttribute("title", state.activeAddress);
  });
