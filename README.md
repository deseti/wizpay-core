...
WizPay Core is an institutional-grade monorepo for high-velocity stablecoin payroll on Arc. It combines a NestJS orchestration layer, BullMQ-backed task execution, external-wallet signing, and Arc settlement contracts to move USDC with deterministic task state, auditable retries, and low operational overhead.
...
| External-wallet execution | Prepare transfers, payroll, and payment flows for user-signed submission | Connected wallets, Arc Mainnet |
...
- Arc-native execution surface: the platform is built around Arc settlement constraints and Arc Mainnet direct-USDC flows rather than retrofitting generic EVM payout assumptions.
- External-wallet-first orchestration: connected external wallets are the sole signing path, not an afterthought integration.
...
```text
apps/
	backend/      NestJS task orchestrator, queue workers, execution adapters
...
- FX, swap, bridge, and liquidity paths flow through the same task model, which preserves a single audit surface across external-wallet execution and contract-mediated settlement.
...
- `WizPayMainnetV2.sol` provides direct-USDC payroll settlement with fee controls, pause controls, and deterministic event emission for Arc Mainnet.
...
1. Open an issue or design note before large behavioral changes, especially anything that alters task semantics, execution assumptions, or settlement guarantees.
...
For Mainnet-facing or ecosystem-facing changes, maintain explicit idempotency guarantees, observable audit logs, and production-safe rollback paths.
