...
export function normalizeAddress(value, label) {
  if (typeof value !== "string" || !isAddress(value))
    fail(`${label} must be an explicit EVM address.`);
...
export function bytes32(value, label) {
  if (typeof value !== "string" || !/^0x[0-9a-fA-F]{64}$/.test(value))
    fail(`${label} must be a bytes32 hex value.`);
  return value.toLowerCase();
}
function uintString(value, label, maximum) {
  if (typeof value !== "string" || !/^(0|[1-9]\d*)$/.test(value))
    fail(`${label} must be a canonical unsigned integer string.`);
  const parsed = BigInt(value);
  if (maximum !== undefined && parsed > maximum)
    fail(`${label} exceeds its maximum.`);
...
export function validateDeploymentInput(raw) {
  ...
  if (
    ...
    raw.deploymentManifestSchemaVersion !==
    ...
    fail(
      "Deployment identity must be WizPayMainnetV2 on arc-mainnet chain 5042 with schema version 3.",
    );
  ...
  if ([owner, feeRecipient, deployer].includes(canonicalUsdc))
    fail(
  ...
  if (
    typeof raw.auditedCommit !== "string" ||
    !/^[0-9a-f]{40}$/.test(raw.auditedCommit)
  )
    fail("auditedCommit must be a lowercase full 40-character Git SHA.");
  if (
    raw.authorizationEvidencePath !== null &&
    (typeof raw.authorizationEvidencePath !== "string" ||
      raw.authorizationEvidencePath.length === 0)
  )
    fail(
      "authorizationEvidencePath must be a repository-relative path or null.",
    );
...
function metadataOf(artifact) {
  ...
  if (!metadata || typeof metadata !== "object")
    fail("Fresh artifact metadata is unavailable.");
  return metadata;
}
export function compilerConfiguration(artifact) {
  ...
  if (!settings || typeof settings !== "object")
    fail("Fresh artifact compiler settings are unavailable.");
  const compiler = {
  ...
  if (
    ...
    !compiler.version.startsWith("0.8.24+") ||
    compiler.optimizer !== true ||
    compiler.optimizerRuns !== "200" ||
    compiler.evmVersion !== "cancun" ||
    compiler.viaIR !== true ||
    typeof compiler.metadataBytecodeHash !== "string" ||
    stableJson(compiler.compilationTarget) !==
      stableJson({ "src/WizPayMainnetV2.sol": "WizPayMainnetV2" }) ||
    !Array.isArray(compiler.remappings)
  )
    fail(
      "Fresh artifact compiler configuration does not match the audited deployment policy.",
    );
  return Object.freeze(compiler);
}
export function compilerDigest(compiler) {
  return canonicalDigest(compiler);
}
...
function materializeRuntimeBytecode(artifact, runtimeBytecode, canonicalUsdc) {
  ...
  if (references.length === 0)
    fail("Runtime immutable references are unavailable.");
  const word = canonicalUsdc.slice(2).toLowerCase().padStart(64, "0");
  const bytes = runtimeBytecode.slice(2).split("");
  for (const reference of references) {
    if (reference.length !== 32 || !Number.isSafeInteger(reference.start))
      fail("Runtime immutable reference is invalid.");
...
function sourceSetDigest(artifact) {
  ...
  if (
    ...
    !sources["src/WizPayMainnetV2.sol"]?.keccak256
  )
    fail("Fresh artifact source provenance is unavailable.");
  return canonicalDigest(
    Object.fromEntries(
      Object.entries(sources)
        .sort(([left], [right]) => left.localeCompare(right))
        .map(([name, value]) => [name, value.keccak256]),
    ),
  );
}
export function encodePlanCore(core) {
  const identityDigest = keccak256(
    encodeAbiParameters(
      parseAbiParameters("uint256, uint256, string, uint256, string"),
      [
        BigInt(core.schemaVersion),
        BigInt(core.deploymentManifestSchemaVersion),
        core.network,
        BigInt(core.chainId),
        core.contract,
      ],
  ...
  return encodeAbiParameters(
    parseAbiParameters(
      "bytes32, bytes32, bytes32, bytes32, bytes32, bytes32, bytes32, bytes32, address, address, address, address, uint256, bytes32",
    ),
    [
      identityDigest,
      core.sourceCommitDigest,
      core.sourceSetDigest,
      core.compilerDigest,
      core.creationBytecodeHash,
      core.runtimeBytecodeHash,
      core.initCodeHash,
      core.constructorDigest,
      core.canonicalUsdc,
      core.deployer,
      core.owner,
      core.feeRecipient,
      BigInt(core.feeBps),
      core.resourceManifestDigest,
    ],
  );
}
export function buildDeploymentPlan(rawInput, artifact, dependencies = {}) {
  ...
  if (!artifact || !Array.isArray(artifact.abi))
    fail("A freshly compiled Foundry artifact is required.");
  if (!dependencies.resourceManifest)
    fail(
      "The readable resource manifest is required; a caller-supplied digest is not accepted.",
    );
  const currentCommit = String(
    dependencies.sourceCommit ??
      execFileSync("git", ["rev-parse", "HEAD"], {
        cwd: dependencies.repositoryRoot ?? REPOSITORY_ROOT,
        encoding: "utf8",
      }),
  )
    .trim()
  ...
  if (
    ...
    currentCommit !== input.auditedCommit
  )
    fail("Current Git HEAD does not equal the explicitly audited commit.");
  const compiler = compilerConfiguration(artifact);
  const creationBytecode = artifactBytecode(
    artifact,
    "bytecode",
    "Creation bytecode",
  );
  const runtimeTemplate = artifactBytecode(
    artifact,
    "deployedBytecode",
    "Runtime bytecode",
  );
  const runtimeBytecode = materializeRuntimeBytecode(
    artifact,
    runtimeTemplate,
    input.canonicalUsdc,
  );
  ...
  const core = {
    ...
    deploymentManifestSchemaVersion: DEPLOYMENT_MANIFEST_SCHEMA_VERSION,
    network: NETWORK,
    chainId: MAINNET_CHAIN_ID,
    contract: CONTRACT,
    sourceCommit: currentCommit,
    sourceCommitDigest: keccak256(toHex(currentCommit)),
    sourceSetDigest: sourceSetDigest(artifact),
    compiler,
    compilerDigest: compilerDigest(compiler),
    canonicalUsdc: input.canonicalUsdc,
  ...
  if (input.authorizationEvidencePath === null) {
    authorization = Object.freeze({
      state: "unavailable",
      reason: "AUTHORIZATION_EVIDENCE_NOT_COMMITTED",
    });
  } else {
    const evidence = loadAuthorizationEvidence(
...
export function assertPlanDigest(plan) {
  ...
  if (keccak256(encodePlanCore(plan)) !== expected)
    fail("Deployment plan digest mismatch.");
  if (keccak256(plan.initCode) !== bytes32(plan.initCodeHash, "initCodeHash"))
    fail("Deployment init code hash mismatch.");
  return true;
}
export function assertPlanMatchesFreshInputs(
  plan,
  rawInput,
  artifact,
  dependencies,
  ...
  if (stableJson(rebuilt) !== stableJson(plan))
    fail("Deployment plan does not match fresh readable inputs.");
  return rebuilt;
}
export function validateMainnetManifest(manifest) {
  ...
  if (manifest.status === "unavailable") {
    if (
      manifest.resourceManifestDigest !== null ||
      manifest.deploymentPlanDigest !== null
    )
      fail("Unavailable manifest digests must be null.");
    if (
      Object.values(manifest.deploymentResult).some(
        (value) => value !== null && value !== "unavailable",
      )
    )
      fail("Unavailable manifest deployment result must remain empty.");
    return true;
  }
  if (manifest.status !== "deployed")
    fail("Mainnet manifest status must be unavailable or deployed.");
  bytes32(manifest.resourceManifestDigest, "resourceManifestDigest");
  bytes32(manifest.deploymentPlanDigest, "deploymentPlanDigest");
  return true;
}
async function cli() {
  const [command, inputPath, resourcePath, outputPath] = process.argv.slice(2);
  if (command === "validate-manifest" && inputPath && !resourcePath) {
    validateMainnetManifest(JSON.parse(await readFile(inputPath, "utf8")));
    process.stdout.write("Mainnet manifest is valid.\n");
    return;
  }
  if (command === "validate-input" && inputPath && !resourcePath) {
  ...
  if (command === "plan" && inputPath && resourcePath && outputPath) {
    const { buildFreshMainnetArtifact } =
      await import("./sync-mainnet-abi.mjs");
    ...
    try {
      const plan = buildDeploymentPlan(
        ...
        fresh.artifact,
        {
          resourceManifest: JSON.parse(await readFile(resourcePath, "utf8")),
  ...
  fail(
    "Usage: validate-input <input.json> | validate-manifest <manifest.json> | plan <input.json> <resource-manifest.json> <new-output.json>",
  );
}
