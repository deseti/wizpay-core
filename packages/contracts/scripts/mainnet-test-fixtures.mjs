...
export const safeSigner = "0x223456789012345678901234567890123456789a";

export async function authorizedFixture({
  productionAuthorization = false,
} = {}) {
  const repositoryRoot = await mkdtemp(
    join(tmpdir(), "wizpay-mainnet-fixture-"),
  );
  const resourceDirectory =
    "packages/contracts/deployments/authoritative-records/fixtures";
  const authorizationDirectory =
    "packages/contracts/deployments/authorization-records/fixtures";
  await mkdir(join(repositoryRoot, resourceDirectory), { recursive: true });
  await mkdir(join(repositoryRoot, authorizationDirectory), {
    recursive: true,
  });
  const rawResources = {
    chainId: { value: 5042 },
    "rpcEndpoints[0]": {
      url: "https://rpc-a.invalid/",
      providerIdentity: "provider-a",
    },
    "rpcEndpoints[1]": {
      url: "https://rpc-b.invalid/",
      providerIdentity: "provider-b",
    },
    explorerUrl: { baseUrl: "https://explorer.invalid" },
    canonicalUsdc: {
      address: base.canonicalUsdc,
      name: "USD Coin",
      symbol: "USDC",
      decimals: 6,
  ...
  for (const [type, value] of Object.entries(rawResources)) {
    const record = {
      ...
      fixtureOnly: true,
      authority:
        type === "canonicalUsdc" || type === "circleBlockchainIdentifier"
          ? "Circle"
          : "Arc",
      network: {
    ...
    await writeFile(
      join(repositoryRoot, recordPath),
  ...
  const manifest = {
    ...
    network: "arc-mainnet",
    resources: {
      chainId: officials.chainId,
      rpcEndpoints: [
        officials["rpcEndpoints[0]"],
        officials["rpcEndpoints[1]"],
      ],
      explorerUrl: officials.explorerUrl,
      canonicalUsdc: officials.canonicalUsdc,
      nativeGas: officials.nativeGas,
      erc20UsdcDecimals: officials.erc20UsdcDecimals,
      circleBlockchainIdentifier: officials.circleBlockchainIdentifier,
      deploymentStatus: {
        state: "unavailable",
    ...
    capabilities: {
      directSend: false,
      sameTokenUsdcPayroll: false,
      invoice: false,
      paymentLink: false,
      receiptVerification: false,
      executionIntentDurability: false,
      uniswapSwap: false,
  ...
  const unsignedPlan = buildDeploymentPlan(base, artifact, {
  ...
  const payload = {
    planDigest: unsignedPlan.planDigest,
    chainId: "5042",
    creationBytecodeHash: unsignedPlan.creationBytecodeHash,
    runtimeBytecodeHash: unsignedPlan.runtimeBytecodeHash,
    deployer: unsignedPlan.deployer,
    finalSafeOwner: unsignedPlan.owner,
    feeRecipient: unsignedPlan.feeRecipient,
    feeBps: unsignedPlan.feeBps,
    resourceManifestDigest: unsignedPlan.resourceManifestDigest,
    releaseId,
  };
  ...
  await writeFile(
    join(repositoryRoot, authorizationEvidencePath),
