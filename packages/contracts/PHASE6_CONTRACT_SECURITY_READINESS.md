...
- Send uses canonical USDC `transfer` through the user-controlled wallet paths in `apps/frontend/app/send/page.tsx`, with backend wallet validation and durable execution intents and durable execution intents. No WizPay contract is called.
- Same-token Payroll approves canonical USDC to the configured WizPay address and calls `batchRouteAndPay` in `apps/frontend/hooks/wizpay/useWizPayContract.ts`. The backend validates calldata in `capabilities/capability.service.ts`, then reconciles the receipt in `task/payroll-receipt-verifier.service.ts`.
...
| `REMOVED` | Legacy Testnet payment router, deployment script, recorded Testnet deployments, legacy estimation entries, and history addresses (deleted)                                                                                                                                                                                    | These artifacts contained legacy behavior or recorded legacy network resources. They were prohibited deployment inputs for Mainnet and have been deleted.                              |
| `DEFERRED_FX`  | Legacy swap executor, legacy FX adapters/bridge/engine/interfaces; legacy swap deployment scripts; every FX, RFQ, rate, swap, bridge, liquidity, Permit2, and provider script under `scripts/`                         | Cross-token execution, liquidity, bridges, routers, and provider settlement are outside Phase 6 and must remain disabled for Mainnet.                                 |
...
The Mainnet deployment input, manifest, and required contract do not reference legacy swap executors, legacy FX adapters, Uniswap, EURC, liquidity pools, bridges, or legacy network deployments. Runtime registries continue to mark Mainnet resources unavailable; Phase 6 does not activate them.
